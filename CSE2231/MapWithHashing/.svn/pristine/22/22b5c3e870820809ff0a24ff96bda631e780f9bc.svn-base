...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        //Gets a hash code value for the object and finds the appropriate bucket
        int hash = key.hashCode();
        int bucket = mod(hash, this.hashTable.length);

        //Adds the map pair to the corresponding bucket
    ...
    public final Pair<K, V> remove(K key) {
        ...
        //Gets a hash code value and finds the bucket
        int hash = key.hashCode();
        int bucket = mod(hash, this.hashTable.length);

        //Removes the map pair with the corresponding key from the bucket
    ...
    public final Pair<K, V> removeAny() {
        ...
        boolean flag = true;
        int bucket = 0;
        Map.Pair<K, V> removed = null;

        //Goes through each bucket to find the first of size>0 and removes any pair
        while (bucket < this.hashTable.length && flag) {
            if (this.hashTable[bucket].size() > 0) {
                removed = this.hashTable[bucket].removeAny();
                flag = false;
            }
            bucket++;
        }
    ...
    public final V value(K key) {
        ...
        //Finds and returns the value for the key in the bucket
        V value = this.hashTable[bucket].value(key);
    ...
    public final boolean hasKey(K key) {
        ...
        //Checks whether the bucket contains the given key
    ...
    public final int size() {
        ...
        //Goes through and adds the size of each bucket
        while (bucket < this.hashTable.length) {
            size += this.hashTable[bucket].size();
            bucket++;
        }
