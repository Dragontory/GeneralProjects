...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        /*
         * RL if the size of the tree is > 1, then compare the root of the tree
         * with the smaller of the two children if the root is greater than the
         * smaller child then swap the root with the smaller child recursively
         * sift down the root of the corresponding subtree
         */
        T left;
        T right;
        if (last - top > 1) {
            ...
            if (order.compare(left, right) <= 0) {
                if (order.compare(array[top], left) > 0) {
                    exchangeEntries(array, top, top + 1);
                    siftDown(array, 2 * top + 1, last, order);
                }
            } else {
                if (order.compare(array[top], right) > 0) {
                    exchangeEntries(array, top, top + 2);
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        // TODO - fill in body
        // *** you must use the recursive algorithm discussed in class ***
        if ((array.length > 0) && (2 * top + 1 < array.length)) {
            heapify(array, 2 * top + 1, order); // recursive call to heapify left
            heapify(array, 2 * top + 2, order); // recursive call to heapify right
        }
        siftDown(array, top, array.length - 1, order);
        //siftDown(array, top, top + array.length, order);
    }
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        // i dont know what we use order for
        int i = 0;
        while (q.length() > 0) {
            heap[i] = q.dequeue();
            i++;
    ...
    private void createNewRep(Comparator<T> order) {
        ...
        //do we have to initialize this.heap?
        //this.heap;
